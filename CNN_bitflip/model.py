...
ATTACKED_MODEL_PATH = "attacked_model.pth"
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(16 * 14 * 14, 10)  # MNIST图像尺寸28x28
...
# 3. 模型训练函数
def train_model(model, train_loader, device, epochs=20):  # 设置为20个epoch
...
def main():
    ...
    if os.path.exists(ORIGINAL_MODEL_PATH):
        # 加载已保存的模型
        model = load_model(SimpleCNN, ORIGINAL_MODEL_PATH, device)
    else:
        # 初始化并训练新模型
