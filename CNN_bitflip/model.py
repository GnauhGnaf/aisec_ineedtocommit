...
def bit_flip_attack(model, images, labels, max_flips=10, epsilon=1e-2, top_k=3):
    ...
    for idx in range(B):
        ...
        for flip_idx in range(max_flips):
            ...
            # ----------------------------
            perturb = adv.clone()
            perturb += epsilon  # 全局加小扰动
            pred_orig = torch.softmax(model(adv), dim=1)[0, true_label]
            pred_pert = torch.softmax(model(perturb), dim=1)[0, true_label]
            sensitivities = (pred_orig - pred_pert).abs().cpu()  # 统一标量
            
            # ----------------------------
            # 2. 找 top_k 最敏感像素
            # ----------------------------
            grads = torch.zeros(C,H,W)
            for c in range(C):
                for h in range(H):
                    for w in range(W):
                        if (c,h,w) in flipped_positions:
                            grads[c,h,w] = -1
                        else:
                            grads[c,h,w] = torch.rand(1)  # 简单占位
            flat_idx = torch.argsort(grads.flatten(), descending=True)[:top_k]
            for f in flat_idx:
                c_max, h_max, w_max = np.unravel_index(f.cpu().numpy(), (C,H,W))
                adv[0, c_max, h_max, w_max] *= -1.0
